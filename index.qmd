---
title: "Design and Characterization of a Precision Motion System"
subtitle: "Course of Design of Precision Engineering"
author: "Group C"

date: today
date-format: "D MMMM YYYY"
---

## Project Objectives

:::: {.columns}

::: {.column width="50%"}
1. **Design a precision motion de-amplifier**

![](./include/images/final-model.png){fig-align="center" width="80%"}
:::

::: {.column width="50%"}
2. **Static calibration of a precision motion amplifier**

![](./include/images/mech-1.png){fig-align="center" width="50%"}

:::{.footer-note}
Calibration performed on an existing setup.
:::

:::

::::

# Design a Precision Motion De-Amplifier

## Design Workflow

![](./include/images/design-workflow.png){fig-align="center" width="120%"}

## Product Definition

:::: {.columns}

::: {.column width="50%"}
**Customer needs**:

- Straight input and output motion
- De-amplification ratio $\text{OUT}:\text{IN} = 1:20$
- Deviation from linear relationship $\leq 5\%$
- Reversible motion transmission
:::

::: {.column width="50%"}
**Application constraints**:

- Input stroke $\leq 10 \,\mathrm{mm}$
- Input force $\leq 0.1 \,\mathrm{N}$
- Space envelope $\leq 200 \times 200 \times 20 \,\mathrm{mm}$
- Flat sensing surface $\geq 5 \times 5 \,\mathrm{mm}$
- Kelvin clamp support
:::

::::

## Quality Function Deployment (QFD)

:::: {.columns}

::: {.column width="50%"}

- Translate customer needs into concrete design criteria
- Needs $\rightarrow$ **Requirements** (comprehensible by customers)
- Requirements $\rightarrow$ **Specifications** (engineering language)

::: {.footer-note}
QFD won't be entirely explained, since it was already presented in class.
:::
:::

::: {.column width="50%"}
![](./include/images/qfd.png){fig-align="center" width="100%"}
:::
::::

## Product Decomposition

:::: {.columns}

::: {.column width="50%"}
* Existing systems decomposed into **atomic functions**
* Independent analysis performed by each team member
* Useful to **improve the knowledge** of the topic
* Example shown on the right
:::

::: {.column width="50%"}
![](./include/images/ProductDecomposition1Daniele.png){fig-align="center" width="70%"}
:::

::::

## Functional Decomposition

* **Functional architecture** derived from system requirements
* System decomposed into atomic functions
* **Hierarchical organization** of functional blocks

![](./include/images/functional-decomposition.png){fig-align="center" width="100%"}

## Morphological Combination

* Atomic functions combined into a **morphological map** of the system

![](./include/images/Morphological-combination.png){fig-align="center" width="100%"}

## Theory of The Resulution of Inventive Problems (TRIZ)

:::: {.columns}

::: {.column width="50%"}
* Identification of engineering contradictions
* Separation between conceptual solution and implementation
* Application of the [40 TRIZ inventive principles](https://triz40.com/TRIZ_GB.php)
* **Contradictions derived from system requirements**
:::

::: {.column width="50%"}
![](./include/images/TRIZ-method.png){fig-align="center" width="100%"}
:::

::::

## Example of Contradiction Resolution

![](./include/images/TRIZ-example.png){fig-align="center" width="100%"}

## Concepts Generation

* Product decomposition used as a knowledge base for concept generation
* Morphological map translated into **multiple conceptual solutions**
* Emphasis on **system architecture** rather than implementation details
* Preliminary calculations and simulations used for concept validation

::: {.footer-note}
Each figure is labeled above with its corresponding concept number.
:::

## Single-Stage Mechanical Concept

:::: {.columns}

::: {.column width="50%"}
* Motion amplification achieved through gear transmission
* Gear mechanisms unsuitable for high-precision applications due to **backlash and friction**
:::

::: {.column width="50%"}
![](./include/images/concept-ruote-dentate.png){fig-align="center" width="100%"}
:::

::::

## Multi-Stage Compliant Mechanism Concepts

:::: {.columns}
::: {.column width="50%"}
* Multi-stage motion transmission and amplification
* **Elastic deformation** enables high repeatability
* Straight-line motion achieved through **structural symmetry**
* Motion amplification through **lever mechanisms**
* Linear input-output relationship enabled by **lever kinematics**
:::
::: {.column width="50%"}
![](./include/images/double-compound-1.png){fig-align="center" width="100%" style="margin-bottom: 20px;"}

![](./include/images/double-compound-2.png){fig-align="center" width="100%"}
:::
::::

## Single-Stage Compliant Mechanism Concepts

* Single-stage mechanism **integrating** transmission and amplification
* Same kinematic and compliant principles as the multi-stage architectures

:::: {.columns}
::: {.column width="50%"}
![](./include/images/concept-daniele.png){fig-align="center" width="90%"}
:::
::: {.column width="50%"}
![](./include/images/concept-paolo.png){fig-align="center" width="60%"}
:::
::::

## Single-Stage Compliant Mechanism Concepts

:::: {.columns}
::: {.column width="50%"}
![](./include/images/concept-leonardo.png){fig-align="center" width="70%"}
:::
::: {.column width="50%"}
![](./include/images/concept-isacco.png){fig-align="center" width="80%"}
:::
::::

## Robust Decision-Making

:::: {.columns}
:::: {.column width="50%"}
* System requirements adopted as **decision criteria**
* Belief maps used by each group member to evaluate the capability of each concept to satisfy the design criteria
* Decision matrix used for concept ranking and comparison
:::
::: {.column width="50%"}
![](./include/images/beliefmap.png){fig-align="center" width="60%"}
:::
::::

![](./include/images/DecisionMatrix.png){fig-align="center" width="65%"}

## Selected Concept

:::: {.columns}
::: {.column width="50%"}
* **Single-Stage Compliant Mechanism Concept 2** selected from the decision matrix evaluation
* The selected concept was then further analysed and modelled
:::
::: {.column width="50%"}
![](./include/images/concept-isacco.png){fig-align="center" width="80%"}
:::
::::

## Modeling Workflow

![](./include/images/modeling.png){fig-align="center" width="100%"}

## Kinematic Design
:::: {.columns}
::: {.column width="50%"}
* **Lever-based mechanism architecture**:
  - linear displacement de-amplification
  - reversible kinematic behaviour
* Straight-line motion ensured by **structural symmetry**
:::
::: {.column width="50%"}
![](./include/images/final-model.png){fig-align="center" width="100%"}
:::
::::

## Dynamic Design

:::: {.columns}
::: {.column width="50%"}
- **Elastic deformation** enables:
  - high repeatability
  - system reversibility
- **Flexure hinges** adopted to implement joints
- Hinge optimization:
  - Leaf hinges: <span style="color: green;">low stiffness</span>, <span style="color: red;">limited angular excursion</span>
  - Circular hinges: <span style="color: red;">high stiffness</span>, <span style="color: green;">high angular excursion</span>
:::
::: {.column width="50%"}
![](./include/images/final-model.png){fig-align="center" width="100%"}
:::
::::

## Material Selection

- Material selection supported by Ashby Map
- **High yield strength** identified as a key requirement $\Rightarrow$ **Beryllium-Copper** selected to avoid plastic deformation

![](./include/images/chetwynd_histograms.png){fig-align="center" width="100%"}

## Simulation Results

:::: {.columns}
::: {.column width="50%"}
|  |  |
|---|---|
| **Gain OUT : IN** | $1:20$ at $y_{in}=10\,\mathrm{mm}$ |
| **Maximum space envelope** | $199.5 \times 174.87 \times 8\ \mathrm{mm}$ |
| **Maximum linearity deviation** | $0.7\%\ (0.0035\ \mathrm{mm})$ |
| **Maximum input force** | $1.07\ \mathrm{N}$ |

* High vibration attenuation behaviour
* Reasonance at $30 \,\mathrm{rad/s} \approx 4.77 \,\mathrm{Hz}$

::: {.footer-note}
The deviation from linearity is calculated over the full-scale ($FS$) output range:
$$
\Delta_{\text{linearity}} = \frac{\max(|y_{\text{sim}} - y_{\text{interp}}|)}{FS} \times 100
$$
:::
:::
::: {.column width="50%"}
![](./include/images/in-out.png){fig-align="center" width="70%" style="margin-bottom: 20px;"}

![](./include/images/FR.png){fig-align="center" width="70%"}
:::
::::

# Static Calibration of a Precision Motion Amplifier

## Measured System and Experimental Setup

:::: {.columns}
::: {.column width="50%"}
* Kelvin-clamp precision mounting
* Precision-screw manual actuation
* LIF-based input/output displacement measurement
:::
::: {.column width="50%"}
![](./include/images/mech-1.png){fig-align="center" width="80%"}
:::
::::

## Effect of Noise

:::: {.columns}
::: {.column width="50%"}
* **Negligible noise contribution**:
    - Sensor drift: $\sim 10^0 \,\mathrm{nm/s}$
    - Stochastic contribution: $\sim 10^1 \,\mathrm{nm}$ with $95\%$ confidence level

* ACF and FFT analyses therefore considered unnecessary

::: {.footer-note}
Operating displacements were on the order of $\sim 10^{0}\,\mathrm{mm}$ (input) and $\sim 10^{-2}\,\mathrm{mm}$ (output); hence, noise contribution was considered negligible.
:::
:::
::: {.column width="50%"}
![](./include/images/noise-drift.png){fig-align="center" width="80%" style="margin-bottom: 20px;"}

![](./include/images/noise-resid.png){fig-align="center" width="80%"}
:::
::::

## Static Calibration Results

:::: {.columns}
::: {.column width="50%"}
* **Linear second-order model** adopted to fit data:
$$
y_{i,k} = \beta_0 + \beta_{k} + \beta_1 x_{i,k} + \beta_2 x^2_{i,k} + \varepsilon_{i,k} 
$$
where $i$ index of measurement and $k$ index of motion direction.


* Residuals normality: *p*-value $= 0.15$ (Shapiro-Wilk test)

* Maximum deviation from linearity $\Delta_{\text{linearity}} = 0.01\%$

:::
::: {.column width="50%"}
:::{#tbl-regression tbl-colwidths="[40,30,30]"}
| Term                          | Estimate              | Std. Error           |
|:-----------------------------:|:---------------------:|:--------------------:|
| $\beta_0\ \mathrm{(mm)}$      | $-3.61 \cdot 10^{-4}$ | $6.28 \cdot 10^{-6}$ |
| $\beta_{forward}\ \mathrm{(mm)}$ | $-4.02 \cdot 10^{-5}$ | $3.24 \cdot 10^{-6}$ |
| $\beta_1\ \mathrm{(adim)}$    | $4.89 \cdot 10^{-2}$  | $5.16 \cdot 10^{-6}$ |
| $\beta_2\ \mathrm{(1/mm^2)}$  | $-1.67 \cdot 10^{-4}$ | $9.51 \cdot 10^{-7}$ |

Table: Regression coefficients and standard errors.
:::

::: {.footer-note}
Similarly to the modeling section, the deviation from linearity is calculated over the full-scale (FS) output range:
$$
\Delta_{\text{linearity}} = \frac{|y_i - \hat{y}_i|}{FS} \times 100 .
$$
:::
:::
::::

## Calibration Conclusions
- Negligible offset and direction bias coefficients ($\beta_0$, $\beta_{\mathrm{forward}}$): **limited hysteresis effects**
- Minimal quadratic contribution $\beta_2$
- Dominant linear coefficient $\beta_1 = 4.89 \cdot 10^{-2} \approx \frac{1}{20}$, confirming **high linearity**

:::: {.columns}
::: {.column width="50%"}
![](./include/images/calibration.png){fig-align="center" width="100%"}
:::
::: {.column width="50%"}
![](./include/images/residuals.png){fig-align="center" width="100%"}
:::
::::

## AI Usage

AI-based tools were employed to support layout refinement, visual content generation, and presentation optimization.

In particular:

- [ChatGPT by OpenAI](https://openai.com/chatgpt) was used for language refinement, layout optimization, and visual content support;

- [Gemini via NotebookLM](https://notebooklm.google.com/) was used to generate the presentation video.

**All generated materials were reviewed and validated by the authors**.

# Thank you for your attention!